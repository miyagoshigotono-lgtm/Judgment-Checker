...
function startReactionTest() {
  ...
  document.querySelectorAll('.target-btn').forEach(btn => {
    ...
    btn.addEventListener('touchstart', e => {
      e.preventDefault();
      onTargetTap(parseInt(btn.dataset.idx));
    }, { passive: false });
...
function resetAllTargets() {
  document.querySelectorAll('.target-btn').forEach(b => {
    b.className = 'target-btn waiting';
  });
}
...
function scheduleReaction() {
  ...
  state.reactionTimer = setTimeout(() => {
    // ランダムに1つ選んで光らせる
    state.activeTargetIdx = Math.floor(Math.random() * 4);
    const activeBtn = document.querySelector(`.target-btn[data-idx="${state.activeTargetIdx}"]`);
    activeBtn.className = 'target-btn ready';
    status.textContent = 'タップ！';
    state._reactionStart = Date.now();
    // 80msバッファでフライング誤判定を防ぐ
    setTimeout(() => { state.reactionWaiting = true; }, 80);
  }, delay);
}

function onTargetTap(idx) {
  ...
  if (!state.reactionWaiting) {
    ...
    resetAllTargets();
    const tappedBtn = document.querySelector(`.target-btn[data-idx="${idx}"]`);
    tappedBtn.className = 'target-btn too-early';
    resultEl.textContent = 'フライング';
    resultEl.className = 'reaction-result too-early';
    state.reactionTimes.push(600);
    state.reactionIndex++;
    updateReactionUI();

    if (state.reactionIndex >= state.reactionTotal) {
      setTimeout(finishReaction, 800);
    } else {
      setTimeout(scheduleReaction, 1200);
    }
    return;
  }
  ...
  if (idx !== state.activeTargetIdx) {
    // 違うボタンを押した：ペナルティ（反応時間+200ms）
    state.reactionTimes.push(Math.min(rt + 200, 700));
    resultEl.textContent = `違う！ +200ms`;
    resultEl.className = 'reaction-result too-early';
  } else {
